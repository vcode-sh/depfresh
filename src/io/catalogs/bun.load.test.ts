...
describe('bunCatalogLoader.load', () => {
  ...
  it('skips peers catalog by default when --peer is disabled', async () => {
    ...
    const sources = await bunCatalogLoader.load(testDir, { ...baseOptions, peer: false })
  ...
  it('includes peers catalog when --peer is enabled', async () => {
    ...
    const sources = await bunCatalogLoader.load(testDir, { ...baseOptions, peer: true })
  ...
  it('loads catalogs from the nearest ancestor when cwd is a nested subdirectory', async () => {
    ...
    expect(sources).toHaveLength(1)
    expect(sources[0]!.filepath).toBe(realpathSync(join(testDir, 'package.json')))
    expect(sources[0]!.deps[0]!.name).toBe('react')
  })
})
