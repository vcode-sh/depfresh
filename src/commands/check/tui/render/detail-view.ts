import c from 'ansis'
import { arrow, colorDiff, padEnd, sanitizeTerminalText } from '../../../../utils/format'
...
export function renderDetailView(state: TuiState): string[] {
  ...
  const header = `  ${sanitizeTerminalText(dep.name)}  ${sanitizeTerminalText(dep.currentVersion)}${arrow()}?`
  ...
  const distTags = Object.entries(dep.pkgData.distTags)
    .map(
      ([tag, version]) => `${sanitizeTerminalText(tag)}${arrow()}${sanitizeTerminalText(version)}`,
    )
    .join(' | ')
  ...
  if (dep.pkgData.homepage) {
    lines.push(fitLine(`  Homepage: ${sanitizeTerminalText(dep.pkgData.homepage)}`, state.termCols))
  }
  lines.push('')
...
export function renderDetailVersionLine(
  ...
  const versionText = padEnd(sanitizeTerminalText(version.version), versionWidth)

  let line = `  ${pointer} ${versionText}  ${colorDiff(version.diff)}`
  if (version.age) line += `  ${colorAge(version.age)}`
  if (version.distTag) line += `  ${c.cyan(sanitizeTerminalText(version.distTag))}`
  if (version.explain) line += `  ${c.gray(sanitizeTerminalText(version.explain))}`
  if (version.deprecated) line += `  ${c.red('deprecated')}`
  if (version.signaturePresence === 'absent') {
    line += `  ${c.yellow('no-signature-metadata')}`
  }
  if (version.nodeEngines) {
    line += `  ${c.gray(`node ${sanitizeTerminalText(version.nodeEngines)}`)}`
  }
