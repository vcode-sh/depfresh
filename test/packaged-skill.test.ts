...
else if (cli[0] === 'capabilities') process.stdout.write('{"contract":"depfresh.capabilities","schemaVersion":2}')
else if (cli[0] === 'inspect') process.stdout.write('{"contract":"depfresh.inspect","schemaVersion":1,"risks":[],"errors":[]}')
else if (cli[0] === 'plan') {
  process.stdout.write('{"contract":"depfresh.plan","schemaVersion":2,"operations":[{}],"summary":{"blocked":0,"unknown":0,"errors":0},"risks":[],"selection":{"requests":[],"summary":{}}}')
  process.exit(1)
...
describe('packaged depfresh skill commands', () => {
  ...
  it('executes every packaged example and CI shell block with inert argv', () => {
    ...
    const calls = readFileSync(current.log, 'utf8')
      ...
      .split('\n')
      .map((line) => JSON.parse(line) as { command: string; args: string[] })
    expect(calls.some((call) => call.args.includes('--exclude-workspace'))).toBe(true)
    expect(calls.some((call) => call.args.includes('--exclude-catalog'))).toBe(true)
  })
})
