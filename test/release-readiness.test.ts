...
describe('2.1.2 release readiness', () => {
  ...
  it('reuses one tested dist artifact and passes one exact tarball to Windows', () => {
    ...
    expect(verifier).toContain("? ['/d', '/s', '/c',")
    expect(verifier).toContain('const windowsCommand = `""%DEPFRESH_COMMAND_SHIM%"')
    expect(verifier).toContain('windowsVerbatimArguments: windows')
    expect(verifier).not.toContain('call "%DEPFRESH_COMMAND_SHIM%"')
    expect(verifier).toContain('stderrBytes=')
    expect(verifier).not.toContain(['$', '{result.stderr}'].join(''))
